# tarea7-pila-Herrera-Cifuentes
1. Registra los siguientes 6 eventos en orden:
   - "Motor de estribor encendido"
   - "Velocidad warp alcanzada"
   - "Señal de comunicación estable"
   - "Anomalía detectada en sector 7"
   - "Escudos al 40%"
   - "ERROR CRÍTICO: fallo en sistema de navegación"

2. Consulta e imprime el último evento registrado.

3. Si el último evento contiene la palabra "ERROR",
   ejecuta el protocolo de revisión:
   elimina los últimos 3 eventos e imprimir cada uno
   que se va removiendo.

4. Imprime el estado actual de la bitácora:
   - Total de eventos restantes
   - Cuál es ahora el evento en la cima
  
 5.Compilar y Ejecutar:
   - javac src/Main.java
   - java -cp src Main
